...
module.exports = {
  findAll: function(req, res) {
    AutoEntry.find({ liftrange: { $ne: "none",  }, liftHide:false }).sort({ date: -1 })
  ...
  bringBackLift: function(req, res) {
    var vin = req.body.vin.length;
    if (vin === 6){
      var changes = {liftHide:false}
 AutoEntry.findOneAndUpdate({ lastsix:req.body.vin }, {$set:changes},{ new: true }).collation( { locale: 'en', strength: 2 } )
  ...
  update: function(req, res) {
    ...
    AutoEntry.findOneAndUpdate({ _id: req.params.id, liftHide:false }, {$set:changes},{upsert:true})
      .then((dbModel) => {
        console.log("the model is", dbModel );
        res.send(dbModel)})
      .catch(err => res.status(422).json(err));
